...
export default function ResultScreen({
  ...
  // Standard WPM: (Correct / 5) * (60 / Time)
  const minutes = duration / 60;
  const netWpm = Math.round(correctKeyCount / 5 / minutes);
  const kpm = Math.round(correctKeyCount / minutes);
  ...
  const timeStr = `${mm}:${pad(ss)}.${pad(ms)}`;

  const accuracy =
    totalKeystrokes > 0
      ? Math.round((correctKeyCount / totalKeystrokes) * 100)
      : 0;
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className={`text-7xl md:text-8xl font-zen-old-mincho tracking-tighter mb-2 ${color} drop-shadow-[0_0_30px_rgba(255,255,255,0.3)]`}
      >
        {grade}
      </motion.div>
      ...
      <div className="grid grid-cols-2 gap-4 w-full mb-6 md:mb-8">
        {/* Main Stats */}
        <div className="col-span-1 bg-white/5 border border-white/10 rounded-lg p-4 md:p-5 flex flex-col items-center justify-center backdrop-blur-sm">
        ...
        <div className="col-span-2 grid grid-cols-3 gap-3">
          <div className="bg-white/5 border border-white/10 rounded-lg p-3 md:p-4 flex flex-col items-center">
          ...
          <div
            className="bg-white/5 border border-white/10 rounded-lg p-3 md:p-4 flex flex-col items-center cursor-pointer hover:bg-white/10 transition-colors"
            onClick={handleShare}
          >
            <span className="text-[8px] md:text-[10px] text-subtle-gray uppercase tracking-widest">
              Share
            </span>
            <Share2 className="w-4 h-4 md:w-5 md:h-5 mt-1 text-sakura" />
          </div>
        </div>
      </div>
